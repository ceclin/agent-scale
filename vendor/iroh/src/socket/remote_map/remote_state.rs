...
struct State {
    ...
    /// Paths that failed on one or more connections and must not be selected again until
    /// each affected connection establishes a replacement path.
    temporarily_unusable_paths: FxHashMap<transports::FourTuple, FxHashSet<ConnId>>,
...
impl RemoteStateActor {
    ...
    pub(super) fn new(
        ...
        Self {
            ...
            state: State {
                ...
                temporarily_unusable_paths: Default::default(),
    ...
    fn handle_msg_add_connection(
        ...
        if let Some(path) = conn.path(PathId::ZERO) {
            ...
            if let Some(path_remote) = path_remote {
                self.state.clear_path_failure(&path_remote, conn_id);

                if !path_remote.is_relay() && conn.side().is_client() {
    ...
    fn handle_connection_close(&mut self, conn_id: ConnId, closed: Closed) {
        ...
        for failed_on in self.state.temporarily_unusable_paths.values_mut() {
            failed_on.remove(&conn_id);
        }
        self.state
            .temporarily_unusable_paths
            .retain(|_, failed_on| !failed_on.is_empty());
    ...
    fn handle_path_event(&mut self, conn_id: ConnId, event: Result<NoqPathEvent, noq::Lagged>) {
        ...
        match event {
            NoqPathEvent::Established { id: path_id, .. } => {
                ...
                self.state
                    .register_and_configure_path(conn_id, conn_state, &path);
                if let Some(network_path) = conn_state.paths.get(&path_id) {
                    self.state.clear_path_failure(network_path, conn_id);
                }
                self.select_path();
            }
            NoqPathEvent::Abandoned { id, reason, .. } => {
                ...
                // A path is shared by all connections to a remote, but path health is not:
                // e.g. a large blob stream can fail while a small control stream keeps its
                // direct path alive.  Do not let the healthy connection make the actor select
                // the failed path again for the connection that just abandoned it.
                if self.state.selected_path.as_ref() == Some(&network_path) {
                    self.state
                        .temporarily_unusable_paths
                        .entry(network_path.clone())
                        .or_default()
                        .insert(conn_id);
                }
    ...
    fn select_path(&mut self) {
        ...
        let selected_addr = {
            let ctx = PathSelectionContext::new(
                current_path,
                &self.connections,
                &self.state.temporarily_unusable_paths,
            );
...
impl State {
    fn clear_path_failure(&mut self, network_path: &transports::FourTuple, conn_id: ConnId) {
        let Some(failed_on) = self.temporarily_unusable_paths.get_mut(network_path) else {
            return;
        };
        failed_on.remove(&conn_id);
        if failed_on.is_empty() {
            self.temporarily_unusable_paths.remove(network_path);
        }
    }
...
enum PathsSource<'a> {
    Live {
        ...
        temporarily_unusable_paths: &'a FxHashMap<transports::FourTuple, FxHashSet<ConnId>>,
    },
    #[cfg(test)]
    Test {
                ...
                connections
                    ...
                    .flat_map(|(state, conn)| {
                        state
                            .paths
                            .iter()
                            .filter(|(_, addr)| !temporarily_unusable_paths.contains_key(*addr))
