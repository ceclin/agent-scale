...
#[cfg(not(windows))]
pub(crate) type BlobFile = File;
#[cfg(windows)]
pub(crate) type BlobFile = RandomAccessFile;

/// Use positioned_io's efficient Windows wrapper for complete blob reads.
/// Direct `File::read_at` creates a memory mapping for every small blob chunk.
pub(crate) fn into_blob_file(file: File) -> io::Result<BlobFile> {
    #[cfg(not(windows))]
    {
        Ok(file)
    }
...
impl<T: ReadAt> ReadBytesAt for FixedSize<T> {
